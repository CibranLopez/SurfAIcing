...
**Hybrid ML-IAP/DFT surface-energy ranking.** For each generated slab, `SurfAIcing` first checks whether a completed DFT calculation is available. If not, it relaxes the structure using the same ML-IAP used for the bulk and evaluates a single-point energy. ML-IAP- and DFT-evaluated slabs can therefore be combined in the same ranking without modifying the analysis code. The surface formation energy is calculated from the slab energy, the bulk energy per formula unit, and the surface area, following the standard thermodynamic definition: the energy cost per unit area of exposing two free surfaces. All slabs generated from a given bulk structure are ranked from most to least stable and visualized together, so that the most likely surface terminations for a given material can be identified before any further, more expensive property is computed on them.
...
Adsorption energies are calculated by subtracting the replica-scaled bare-surface energy and the appropriate reference energy—a free H atom or half the energy of a free H$_2$ molecule—from the energy of the adsorbed configuration. The same ML-IAP/DFT-agnostic energy-lookup mechanism used for surface ranking is applied here.
...
For each slab, the planar-averaged potential is divided into bulk-like and vacuum regions. A user-defined fraction near the boundaries of each region is discarded to avoid edge artifacts, and the mean potential is calculated in both regions. The slab VBM relative to vacuum is then obtained by combining the bulk VBM with the potential offsets between the bulk-like and vacuum regions. Adding the band gap gives the conduction-band minimum (CBM).
...
$$
E_{\text{adhesion}} = (E_{\text{interface}} - E_{\text{film}} - E_{\text{substrate}})/A,
$$

using the same ML-IAP/DFT-agnostic energy-lookup mechanism as the other modules. Interfaces can therefore either be ranked using ML-IAPs or passed directly to DFT.

`SurfAIcing` has already been used to generate surfaces and calculate vacuum-referenced band alignments for a systematic study of band-edge engineering in pnictogen chalcohalide (MChX) solid solutions for photocatalytic and photovoltaic applications [@lopez2026bandedge]. That study builds on earlier machine-learning-aided first-principles predictions of MChX optoelectronic properties [@lopez2024mlaided].
